...
const validatePhone = (phone: string | null) => {
    if (phone && phone !== '') return isMobilePhone(phone.replace(/ /g, ''));
};
...
const Welcome: NextPageWithLayout = () => {
        ...
        if (data.length) {
            for (let i in data)
                notify({
                    ...
                    content: `${data[i].username} wants to be you friend`,
                });
        }
    };
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (
            formData.phone &&
            !isMobilePhone(formData.phone.replace(/ /g, ''))
        ) {
        ...
        if (!tmp.username && !tmp.email && !tmp.phone) {
            if (formData.phone === '') editUser({ ...formData, phone: null });
    ...
    const activateTfa = async () => {
        ...
        if (req.status === 200) {
            setAlert({
                type: 'success',
                content: '2FA activated successfully',
            });
    ...
    const deactivateTfa = async () => {
        ...
        if (req.status === 200) {
            setAlert({
                type: 'success',
                content: '2FA deactivated successfully',
            });
            setTfaStatus('disabled');
        } else {
            setAlert({
                type: 'error',
                content: 'Error while deactivating 2FA!',
            });
            setTfaStatus('enabled');
        }
    };
    ...
    return (
        <div
            className="min-h-screen text-white bg-gray-900 grow"
            id="main-content"
        >
            <div
                        ...
                        <div className="flex flex-col py-12 gap-y-10">
                            ...
                            >
                                ...
                                <div className={inputGroupClassName}>
                                    ...
                                    <small>
                                        We will use that data for two factor
                                ...
                                <div className={inputGroupClassName}>
                                    ...
                                    <small>
                                        Confirm each connection to your account
