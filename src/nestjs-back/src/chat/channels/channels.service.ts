...
export class ChannelsService {
  ...
  async findOutIfUserIsMuted(channelId: number, userId: number) {
    return await this.channelPunishmentService.isUserCurrentlyMuted(
      channelId,
      userId,
    );
  }
  ...
  async findOutIfUserIsBanned(channelId: number, userId: number) {
    return await this.channelPunishmentService.isUserCurrentlyBanned(
      channelId,
      userId,
    );
  }
  ...
  async create(createChannelDto: CreateChannelDto) {
    if (createChannelDto.password) {
      createChannelDto.password = await hashPassword(
    ...
    return await this.channelsRepository.save(channel).catch(() => {
      throw new Error(
        `Group '${createChannelDto.name}' already exists. Choose another name.`,
      );
    });
  }
