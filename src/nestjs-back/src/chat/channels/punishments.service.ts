...
import {
  ChannelPunishment,
  PunishmentType,
} from './entities/punishment.entity';
...
export class PunishmentsService {
  ...
  async punishUser(
    ...
    if (!channel) {
      throw new Error(
        `Could not punish user since there is no channel with id ${channelId}`,
      );
    }
    ...
    if (!punishedUser) {
      throw new Error(
        `Could not punish user: no user with id ${punishedUserId}`,
      );
    }
    ...
    const punishmentEndDate = new Date(
      punishmentStartDate.getTime() + durationInSeconds * 1000,
    );
  ...
  async getActiveUserPunishments(
    channelId: number,
    userId: number,
    options: GetPunishmentOptions = {},
  ) {
    const channel = await this.channelRepository.findOne(channelId);
    if (!channel) {
      throw new Error(
        `Could not check punishment: no channel with id ${channelId}`,
      );
    }
    ...
    const userPunishments = await this.channelPunishmentRepository
  ...
  async isUserCurrentlyPunished(
    ...
    if (!channel) {
      throw new Error(
        `Could check punishment: no channel with id ${channelId}`,
      );
    }
    ...
    const punishmentCount = await this.channelPunishmentRepository
      .createQueryBuilder('punishment')
      .leftJoin('punishment.punishedUser', 'punished')
      .leftJoin('punishment.punishedByUser', 'punisher')
      .leftJoin('punishment.channel', 'channel')
  ...
  async isUserCurrentlyMuted(channelId: number, userId: number) {
    return this.isUserCurrentlyPunished(channelId, userId, {
      onlyType: 'mute',
    });
  }
