...
export class ChannelPunishmentsController {
  constructor(private readonly channelPunishmentService: PunishmentsService) {}
  ...
  getPunishmentsForUser(
    ...
    return this.channelPunishmentService.getActiveUserPunishments(
      channelId,
      userId,
      { onlyType: 'mute' },
    );
  ...
  punishUser(
    @Param('channelId') channelId: number,
    @Query('punishedId') punishedId: number,
    @Query('punisherId') punisherId: number,
  ) {
    ...
    if (punisherId === punishedId) {
      throw new BadRequestException(
        'punisherId should be different from punishedId',
      );
    }
  ...
  @Get('/is-punished')
  async findOutIfUserIsPunished(
    @Param('channelId') channelId: number,
    @Query('userId') userId: number,
  ) {
    const isPunished =
      await this.channelPunishmentService.isUserCurrentlyPunished(
        channelId,
        userId,
      );
  ...
  @Get('/is-muted')
  async findOutIfUserIsMuted(
    @Param('channelId') channelId: number,
    @Query('userId') userId: number,
  ) {
    const isMuted = await this.channelPunishmentService.isUserCurrentlyMuted(
      channelId,
      userId,
    );
  ...
  @Get('/is-banned')
  async findOutIfUserIsBanned(
    @Param('channelId') channelId: number,
    @Query('userId') userId: number,
  ) {
    const isBanned = await this.channelPunishmentService.isUserCurrentlyBanned(
      channelId,
      userId,
    );
