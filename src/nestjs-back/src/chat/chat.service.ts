...
export class ChatService {
  ...
  async checkChannelPassword(channelId: number, password: string) {
    ...
    if (channel && channel.privacy === 'protected') {
      const chanPassword = await this.channelsService.getChannelPassword(
        channelId.toString(),
      );
  ...
  async checkIfUserIsBanned(channelId: number, userId: number) {
    const isBanned = await this.channelsService.findOutIfUserIsBanned(
      channelId,
      userId,
    );
  ...
  async checkIfUserIsMuted(channelId: number, userId: number) {
    const isMuted = await this.channelsService.findOutIfUserIsMuted(
      channelId,
      userId,
    );
