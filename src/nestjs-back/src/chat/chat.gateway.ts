...
export class ChatGateway implements OnGatewayInit, OnGatewayConnection {
  ...
  async handleGmSubmit(
    ...
    try {
      ...
      await this.chatService.checkIfUserIsMuted(
        data.channel.id,
        data.author.id,
      );
  ...
  async handleLeaveChannel(
    ...
    try {
      const channel = await this.chatService.getChannelData(channelId);
      const user = await this.chatService.removeUserFromChannel(
        channel,
        userId,
      );
      const message = await this.chatService.addMessageToChannel({
  ...
  async handleJoinProtectedChannel(
    ...
    try {
      ...
      const isInChan = await this.chatService.userIsInChannel(
        channelId,
        userId,
      );
  ...
  async handleMakeAdmin(
    ...
    try {
      ...
      this.logger.log(
        `User [${userId}] is now admin in Channel [${channel.name}]`,
      );
      ...
      if (chatUser) {
        this.server
          .to(chatUser.socketId)
          .emit('chatInfo', `You are now admin in ${channel.name}.`);
      }
    } catch (e) {
      this.server.to(client.id).emit('chatError', e.message);
    }
  ...
  async handleRemoveAdmin(
    ...
    try {
      ...
      this.logger.log(
        `User [${userId}] no longer admin in Channel [${channel.name}]`,
      );
      ...
      if (chatUser) {
        this.server
          .to(chatUser.socketId)
          .emit('chatInfo', `You are no longer admin in ${channel.name}.`);
      }
    } catch (e) {
      this.server.to(client.id).emit('chatError', e.message);
    }
  ...
  async handleKickUser(
    ...
    if (adminId === userId) {
      throw new WsException(
        'If you want to leave the channel, go to Channel settings.',
      );
    }
    try {
      ...
      if (chatUser) {
        this.server
          ...
          .emit(
            'kickedFromChannel',
            `You have been kicked from ${channel.name}.`,
          );
      ...
      this.logger.log(
        `User [${userId}] was kicked from Channel [${channelId}]`,
      );
    } catch (e) {
      this.server.to(client.id).emit('chatError', e.message);
    }
  ...
  async handlePunishUser(
    ...
    try {
      const channel = await this.chatService.getChannelData(channelId);
      const message = await this.chatService.punishUser(
        channel,
        adminId,
        userId,
        type,
      ...
      if (!dm) {
        ...
        if (friendUser) {
          ...
          this.server
            ...
            .emit('invitedInChat', { from: user.id });
        }
      }
      this.userJoinRoom(client.id, `dm_${dm.id}`);
