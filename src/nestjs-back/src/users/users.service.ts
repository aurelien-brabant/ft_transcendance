...
export class UsersService {
  ...
  async update(id: string, updateUserDto: UpdateUserDto) {
    ...
    if (updateUserDto.wins || updateUserDto.losses || updateUserDto.losses) {
      ...
      let updated = [];
      for (let i in oldFriends) updated.push({ id: oldFriends[i].id });
      ...
      let updated = [];
      for (let i in oldBlocked) updated.push({ id: oldBlocked[i].id });
      ...
      let updated = [];
      for (let i in old) updated.push({ id: old[i].id });
