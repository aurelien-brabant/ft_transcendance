...
export class Ball implements IBall {
  ...
  reset() {
    const dir = this.x < canvasWidth / 2 ? -1 : 1;
    this.x = canvasWidth / 2;
    this.y = canvasHeight / 2;
    this.speed = ballDefaultSpeed;
  ...
  update(secondPassed: number, p1: Player, p2: Player) {
    if (this.r < ballDefaultRadius) this.r += 1;
  ...
  handleCollision(secondPassed: number, p1: Player, p2: Player) {
    ...
    if (
      this.y + this.velocity.dy * secondPassed - this.r <= 0 ||
      this.y + this.velocity.dy * secondPassed + this.r >= 1080
    ) {
    ...
    if (this.collision(secondPassed, p1, p2)) {
      ...
      const p = this.x < canvasWidth / 2 ? p1 : p2;
