...
export default class Room implements IRoom {
  ...
  constructor(
    roomId: string,
    users: User[],
    customisation: { maxGoal?: number } = { maxGoal: 3 },
  ) {
    this.roomId = roomId;
  ...
  isAPlayer(user: User): boolean {
    return (
      this.playerOne.user.username === user.username ||
      this.playerTwo.user.username === user.username
    );
  }
  ...
  removeUser(userRm: User) {
    const userIndex: number = this.players.findIndex(
  ...
  update(): void {
    const secondPassed: number = (Date.now() - this.lastUpdate) / 1000;
    this.lastUpdate = Date.now();
    ...
    if (this.ball.goal === true) {
      this.goalTimestamp = Date.now();
      if (
        this.playerOne.goal === this.maxGoal ||
        this.playerTwo.goal === this.maxGoal
      ) {
        ...
        this.winner =
          this.playerOne.goal === this.maxGoal
            ? this.playerOne.user.username
            : this.playerTwo.user.username;
        this.loser =
          this.playerOne.goal === this.maxGoal
            ? this.playerTwo.user.username
            : this.playerOne.user.username;
